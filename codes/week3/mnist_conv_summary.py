...
with tf.name_scope('conv1'):
    ...
    tf.summary.histogram('actv', conv)
    tf.summary.histogram('actv2', relu)
...
with tf.name_scope('fc2'):
    ...
    tf.summary.histogram('actv', fc)
    tf.summary.histogram('actv2', logits)
    ...
    for epoch in range(n_epochs):
        for itr in range(n_itrs):
            ...
            if (itr % display_step == 0):
                loss_val, acc_val, summary = sess.run([loss, acc, summary_merged], feed_dict={X: train_x,
                                                     Y: train_y,
                                                     keep_prob: 1.0})
